...
var mapSketch = function(p5j){
    ...
    p5j.map = L.map('map').setView([23.5,121], 8); // 經緯度 比例
    p5j.canvas;
    p5j.display = true;
...
*/
  //oop
    ...
    $('#lbutton').click(()=>{
        let url = 'https://spreadsheets.google.com/feeds/list/1FU_OJc81l11adKIV7lKCsejx-bE-fefSVobfMuSUeXA/1/public/values?alt=json';
        p5j.loaded=0;
        //console.log(url);
        p5j.httpGet(url, 'jsonp', false, function(response) {
    ...
    $('#ubutton').click(()=>{
        if(p5j.arrarea.length>0){
          ...
          let exeurl = 'https://script.google.com/macros/s/AKfycbwfi5uTNhH3VdUkIV0hpCjlLyBkk7V0pCXvR00gjVVSVFUI6w_x/exec';          
          let editurl = 'https://docs.google.com/spreadsheets/d/1FU_OJc81l11adKIV7lKCsejx-bE-fefSVobfMuSUeXA/edit#gid=0';
          console.log(lattxt);
